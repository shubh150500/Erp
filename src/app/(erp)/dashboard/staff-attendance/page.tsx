...
async function AdminView() {
  ...
  const lessonsDone = await safeQuery(
    () =>
  ...
  const homeworkCounts = await safeQuery(
    () =>
  ...
  const markedToday = teachers.filter((t) =>
    (t.attendance || []).some((a) => a.date && new Date(a.date).getTime() === today.getTime())
  ).length;

  const rows = teachers.map((t) => {
    const todayRec = (t.attendance || []).find((a) => a.date && new Date(a.date).getTime() === today.getTime());
    return {
      id: t.id,
      ...
      batches: t._count?.batches ?? 0,
      students: (t.batches || []).reduce((s, b) => s + (b._count?.enrollments ?? 0), 0),
  ...
  return (
    <>
      ...
      <div className="grid gap-5 sm:grid-cols-3 mb-7">
        ...
        <StatCard label="Total batches" value={teachers.reduce((s, t) => s + (t._count?.batches ?? 0), 0)} />
      </div>
      ...
      <Panel title="Mark attendance" className="mb-7">
        <TeacherAttendanceMarker
          teachers={teachers.map((t) => ({
            ...
            name: t.user?.name ?? "Teacher",
            subject: t.subject,
            status: (t.attendance || []).find((a) => a.date && new Date(a.date).getTime() === today.getTime())?.status,
          }))}
        />
      </Panel>
...
async function TeacherView({ userId }: { userId: string }) {
  ...
  const attendanceList = teacher.attendance || [];
  const pct = presentPct(attendanceList.map((a) => a.status));
  ...
  return (
    <>
      ...
      <div className="grid gap-5 sm:grid-cols-3 mb-7">
        ...
        <StatCard label="Days recorded" value={attendanceList.length} />
        <StatCard label="Present days" value={attendanceList.filter((a) => a.status === "PRESENT" || a.status === "LATE").length} tone="green" />
      </div>
            ...
            {attendanceList.slice(0, 30).map((a) => (
              <li key={a.id} className="flex items-center justify-between px-5 py-3">
                <span className="text-sm text-navy-600">
                  {a.date ? new Date(a.date).toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short", year: "numeric" }) : "—"}
                </span>
