        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-navy-900/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-[var(--shadow-lift)]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-navy-100">
              <h2 className="font-serif text-xl font-bold text-navy-700">Record expense</h2>
            ...
            {state.ok ? (
              ...
              <form action={action} className="p-6 space-y-4">
                ...
                <Field label="Title" err={state.errors?.title}>
                  <input name="title" className={inputCls} placeholder="e.g. June office rent" />
                </Field>
                ...
                <Field label="Date" err={state.errors?.spentAt}>
                ...
                <Field label="Invoice / bill (optional · PDF or image)" err={state.errors?.invoice}>
                  <input
                    name="invoice"
                    type="file"
                    accept="application/pdf,image/png,image/jpeg,image/webp"
                    className="w-full text-sm text-navy-600 file:mr-3 file:rounded-lg file:border-0 file:bg-navy-700 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-navy-800"
                  />
                </Field>
